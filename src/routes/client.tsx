...
export const Route = createFileRoute("/client")({
  head: () => ({
    meta: [
      { title: "Mijn Dashboard — Adcure Agency" },
      {
        name: "description",
        content:
          "Beheer je ad accounts, wallet en top-ups vanuit je persoonlijke client dashboard.",
      },
      { property: "og:title", content: "Mijn Dashboard — Adcure Agency" },
      {
        property: "og:description",
        content: "Persoonlijk overzicht van je ad accounts en wallet.",
      },
    ],
  }),
...
// Mock client identity (would come from auth in real app)
const CLIENT = {
  name: "Sofia Martinez",
  firstName: "Sofia",
...
const recentTransactions: ClientTransaction[] = [
  ...
  },
  {
    ...
    meta: "Northwind Performance · Meta",
    amount: "−€1,200.00",
    status: "Completed",
    time: "5u geleden",
  },
  ...
  },
  {
    id: "tx5",
    type: "refund",
    title: "Refund verwerkt",
    meta: "Lumen Skincare · Meta",
    amount: "+€120.00",
    status: "Completed",
    time: "3d geleden",
  },
      ...
      <div className="animate-fade-in space-y-8">
        ...
        <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <KpiCard
            ...
            value="€18,240.00"
            delta={{ value: "+€2,500", positive: true }}
            icon={Wallet}
            accent="success"
          />
          <KpiCard
            label="Spend deze maand"
            value="€7,431.62"
            delta={{ value: "+12.4%", positive: true }}
            icon={TrendingUp}
            accent="violet"
            hint="t.o.v. vorige maand"
          />
          <KpiCard
        ...
        <section className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          ...
          >
            ...
            <span className="flex-1">
              ...
              <span className="block text-[11px] text-muted-foreground">
                Vraag een nieuw ad account aan
              </span>
            </span>
            <ArrowUpRight className="h-4 w-4 text-muted-foreground transition group-hover:text-foreground" />
          </button>
...
function RecentTransactions({
  ...
  const iconMap = {
    ...
    refund: {
      icon: CheckCircle2,
      color: "text-primary-glow bg-primary/15 ring-primary/25",
    },
    request: {
      icon: AlertTriangle,
      color: "text-warning bg-warning/15 ring-warning/25",
    },
  } as const;
    ...
    <GlassCard>
      ...
      <ul className="space-y-2">
        {transactions.map((t) => {
          ...
          const amountColor = t.amount.startsWith("+")
            ? "text-success"
            : t.amount.startsWith("−")
              ? "text-foreground"
          ...
          return (
            <li key={t.id}>
              <button
                type="button"
                className="flex w-full items-start gap-3 rounded-xl border border-border bg-background/30 p-3 text-left transition hover:border-border-strong hover:bg-background/60"
              >
                <div
                  className={cn(
                    "mt-0.5 rounded-lg p-2 ring-1 ring-inset",
                    iconMap[t.type].color,
                  )}
                >
...
function AdAccountsPreview({
  accounts,
}: {
  accounts: typeof adAccounts;
}) {
    ...
    <GlassCard>
      ...
      <ul className="space-y-2">
        {accounts.map((a) => (
          <li
            key={a.id}
            className="flex items-center gap-3 rounded-xl border border-border bg-background/30 p-3 transition hover:border-border-strong hover:bg-background/60"
          >
            <div
              className={cn(
                "flex h-9 w-9 shrink-0 items-center justify-center rounded-lg text-[10px] font-semibold uppercase ring-1 ring-inset",
                platformColor[a.platform] ?? "text-muted-foreground bg-muted/40",
              )}
            >
            ...
            <button
              type="button"
              className="inline-flex items-center gap-1.5 rounded-lg bg-primary/15 px-2.5 py-1.5 text-xs font-semibold text-primary-glow ring-1 ring-inset ring-primary/25 transition hover:bg-primary/25"
            >
